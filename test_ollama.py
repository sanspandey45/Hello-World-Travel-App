...
if st.session_state.page == "itinerary":
    ...
    def create_full_itinerary():
        ...
        # Add evening activity if the day ends early
        if current_time.hour < 20:
            evening_activity = "Walk along Lake Ray Roberts "
            itinerary.append({
                "time": current_time.strftime("%I:%M %p"),
    ...
    def handle_chat_input():
        ...
        # 1. Show itinerary command
        if any(cmd in message_lower for cmd in ["show itinerary", "view itinerary", "see itinerary"]):
            bot_response = display_current_itinerary()
        
        # 2. Add event command
        elif "add event" in message_lower:
            event_num = re.search(r"add event (\d+)", message_lower)
            if event_num:
                bot_response = add_event_to_itinerary(event_num.group(1))
            else:
                bot_response = "Please specify which event to add (e.g. 'Add event 1')"
        
        # 3. Add restaurant command
        elif "add restaurant" in message_lower:
            restaurant_num = re.search(r"add restaurant (\d+)", message_lower)
            if restaurant_num:
                bot_response = add_restaurant_to_itinerary(restaurant_num.group(1))
            else:
                bot_response = "Please specify which restaurant to add"
        
        # 4. Remove item command
        elif "remove" in message_lower:
            item_num = re.search(r"remove (\d+)", message_lower)
            if item_num:
                bot_response = remove_from_itinerary(item_num.group(1))
            else:
                bot_response = "Please specify which item to remove"
        
        # If no specific command, use either the original response or Llama 3
        else:
            # First try the original response function
            ...
            if response.startswith("event_request:"):
                ...
                try:
                    ...
                    if llm_response.startswith("event_request:"):
                        event_type = llm_response.split(":")[1]
                        bot_response = handle_event_request(event_type)
                    elif llm_response.startswith("food_request:"):
                        cuisine = llm_response.split(":")[1]
                        bot_response = handle_food_request(cuisine)
